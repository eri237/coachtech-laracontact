...
<div class="container">
  {!! Form::open(['route' => 'confirm', 'method' => 'POST', '@submit.prevent' =>'submitForm']) !!}
  {{ csrf_field() }}
  <div class="form-group row">
    <p class="col-sm-4 col-form-label">お名前<span class="text-danger ml-1 h6">※</span></p>
    <div class="col-sm-4">
      {{ Form::text('last_name', null, ['class' => 'form-control', 'v-model' => 'last_name']) }}
    </div>
    <div class="col-sm-4">
      {{ Form::text('first_name', null, ['class' => 'form-control', 'v-model' => 'first_name']) }}
    </div>
  </div>
  ...
  <div class="form-group row">
    ...
    <div class="col-sm-7">
      ...
      {{ Form::text('zip', null, ['class' => 'form-control', 'v-model' => 'zip','onKeyUp'=>"AjaxZip3.zip2addr('zip', '', 'address', 'address');"]) }}
    </div>
  </div>
  @if ($errors->has('zip'))
  ...
  <div class="form-group row">
    ...
    <div class="col-sm-8">
      {{ Form::textarea('opinion', null, ['class' => 'form-control', 'v-model' => 'opinion']) }}
    </div>
  </div>
...
<script>
  ...
  const app = new Vue({
    ...
    validations: {
      ...
      zip: {
        required: validators.required,
        zipcoderule: rules.new ZipCodeRule()
      },
      address: {
        required: validators.required
      },
